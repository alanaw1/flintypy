...
def _hamming_distance_gmpy(X):
    ...
    ----------
    X : int64 numpy array
        An :math:`N\\times P` array recording :math:`P` features in :math:`N` individuals

    Returns
    -------
    float64 numpy array
        A length :math:`{N \\choose 2}` array containing the pairwise Hamming distances in order :math:`((1, 1), (1, 2), ..., (1, N), (2, 3),\\ldots)`.
...
def _calculate_bin_v_stat(X):
    ...
    -------
    float
        The :math:`V` statistic, a scalar computing the variance of the pairwise Hamming distance between individuals.    
...
def _calculate_real_v_stat(X, p):
    ...
    where :math:`V(\\mathbf{X})` is the scaled variance of :math:`l_p^p` distances 
    ...
    -------
    float
        The :math:`V` statistic, a scalar computing the variance of the pairwise :math:`l_p^p` distance between individuals.    

    '''
    return np.var(pdist(X, 'minkowski', p)**p)/X.shape[1]
...
def _numba_permute(X, block_labels):
    ...
    block_labels : int64 numpy array
        A length :math:`P` array, with entry :math:`i` containing the label of the block that contains feature :math:`i`.
        Blocks are assumed to be labeled 0 to number of blocks - 1.

    Returns
    -------
    numpy array
        An :math:`N \\times P` matrix :math:`\\mathbf{X}'`, which is a permuted version of :math:`\\mathbf{X}`.
...
def _naive_block_permute(X, block_labels, p):
    ...
    -------
    float
        :math:`V(\\mathbf{X}')`, where :math:`\\mathbf{X}'` is a resampled version of :math:`\\mathbf{X}`.
...
def _numba_permute_dists(dists, forward, reverse):
    ...
    underlying labels. Arguments `forward` and `reverse` should be precomputed using
    ``_build_forward`` and ``_build_reverse``.
    ...
    dists : float64 numpy array
        A :math:`B \\times {N \\choose 2}` array, where each column records the pairwise distances across all blocks,
    ...
    reverse : numpy 2D array
        A :math:`{N \\choose 2}\\times 2` mapping from an index in `dists` to the 
        corresponding labels `i` and `j`. Should be
...
def _build_forward(n):
    ...
    distances especially when dealing with high-dimensional (large :math:`P`) arrays.
...
def _build_reverse(n):
    ...
    reverse : int64 2D array 
        An :math:`{N \\choose 2} \\times 2` array, whose entries at row :math:`k`, :math:`(k,0)` and :math:`(k,1)`, 
        are the indices that make up the :math:`k` th pair in the list :math:`((1,1), (1,2), ..., (1,N), (2,3),\\ldots)`
...
def _cache_block_permute(X, 
    ...
    blocks : List of int64 numpy arrays
        List of arrays, with the :math:`k` th array containing the indices of features
        in block :math:`k`.
    nruns : int
    ...
    -------
    array
        Array of floats storing the permutation distribution of the :math:`V` statistic
    ...
    if ((X==0) | (X==1)).all():
        ...
        dists = np.array([pdist(X[:, b], 'minkowski', p)**p for b in blocks])
...
def _convolution_of_chi2(val, w1, w2, d1, d2):
    ...
    Computes :math:`P(X > val)` where :math:`X = w_1 Y + w_2 Z`, where :math:`Y`
    is chi square distributed with :math:`d_1` degrees of freedom,
    ...
    -------
    float
        :math:`1 - CDF = P(X > val)`, the probability that the rv is at least val
...
def _ind_cov(X, p):
    ...
    :math:`\\beta` and :math:`\\gamma` quantities defined in Aw, Spence and Song (2021),
    assuming the :math:`P` features of dataset :math:`\\mathbf{X}` are independent. 
    ...
    -------
    float 1D array
        The three distinct entries of covariance matrix :math:`(\\alpha,\\beta,\\gamma)`, 
        all floats.
...
def _chi2_weights(alpha, beta, gamma, n):
    ...
    from the covariance entries :math:`\\alpha`, :math:`\\beta` and :math:`\\gamma`
    obtained from :math:`\\mathbf{X}`.
    ...
    -------
    1D floats
        Two floats, :math:`w_1` and :math:`w_2`, where :math:`w_1` is the weight for the chi square distribution with :math:`n-1` degrees of 
        freedom and :math:`w_2` is the weight for the chi square distribution with :math:`{n-1 \\choose 2} - 1` degrees of freedom.
...
def _block_cov(X, blocks, p):
    ...
    :math:`\\alpha`, :math:`\\beta` and :math:`\\gamma` quantities, for partitionable
    features that are grouped into blocks. Computes the unique entries of the asymptotic covariance matrix 
    of the pairwise distances in :math:`O(N^2)` time. 
...
def _block_large_p_large_n(X, blocks, p):
    ...
    Computes the large :math:`P`, large :math:`N` asymptotic p-value for :math:`\\mathbf{X}`,
    assuming its :math:`P` features are are independent within specified blocks.
...
def _dist_data_permute(dist_list, nruns):
    ...
    if len(dist_list[0].shape) == 1: 
        print("Distances are in vector form")
        # Get sample size
        n = squareform(dist_list[0]).shape[0]
        # Convert to nd.array
        dists = np.array(dist_list)
    else: 
        print("Distances are in matrix form")
...
def _dist_data_large_p(dist_list):
    ...
    a :math:`B\\times {N \\choose 2}` array, and then using large-:math:`P` theory to 
...
def get_p_value(X, blocks=None, 
    ...
    **A Non-parametric Test of Exchangeability and Homogeneity**
    ...
    :math:`V`, is atypically large under the null hypothesis of exchangeability.
    
    Note the user may tweak the choice of power :math:`p` if they prefer an :math:`l_p^p` 
    distance other than Euclidean (:math:`p=2`). 
    
    Under the hood, the variance statistic :math:`V` is computed efficiently. 
    Moreover, the user can specify their choice of block permutations, large :math:`P`
    asymptotics, or large :math:`P` and large :math:`N` asymptotics. The latter two
    ...
    to use large :math:`P` asymptotics. If :math:`P` or :math:`B` is small, stick
    with permutations. 
    ...
    blocks : List of int64 numpy arrays, optional
        List of arrays, with the :math:`k` th array containing the indices of features
        in block :math:`k`. The default is None.
    ...
    p : float, optional
        The Minkowski power, :math:`l_p^p = (x_1^p+\\ldots+x_n^p)`. The default is 2.
    ...
    NotImplementedError
        This error corresponds to the large :math:`N`, small :math:`P` asymptotics,
        which is currently not implemented.
        
    Returns
    -------
    float
        The p-value computed for the :math:`V` statistic of :math:`\\mathbf{X}`.
    ...
    if ((X==0) | (X==1)).all(): 
        ...
        if np.any(col_sums == 0) or np.any(col_sums == X.shape[0]):
            warnings.warn("There exist columns with all ones or all zeros for binary X.")
...
def dist_data_p_value(dist_list, large_p = False, num_perms = 1000):
    ...
    **A Non-parametric Test for Exchangeability and Homogeneity (Distance List Version)**
    ...
    if (len(dist_list) < 50 and large_p):
        raise IOError('Too few independent distances for approximation to be valid')
